...
export function renderDopamineBar(data, containerId) {
  const { svg, width, height } = createChartSvg(containerId);
  addTitle(svg, data.title, width);

  const x = d3.scaleBand().domain(data.categories).range([0, width]).padding(0.25);
  ...
  svg.append('text').attr('transform', 'rotate(-90)').attr('y', -45).attr('x', -height / 2)
    .attr('text-anchor', 'middle').attr('fill', 'rgba(255,255,255,0.5)').attr('font-size', '11px').text('Dopamine (pg/ml)');

  svg.selectAll('.bar').data(data.values).enter().append('rect')
    .attr('x', (_, i) => x(data.categories[i])).attr('width', x.bandwidth())
